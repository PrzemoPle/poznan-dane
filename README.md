...
etl/                skrypty pobierania i agregacji (czysty Node, bez zależności)
  ...
  fetch-badam.mjs   parser tabel HTML badam.poznan.pl + sklejanie szeregów
  fetch-opendata.mjs katalogi (GraphQL + REST)
  build-aggregates.mjs  agregaty, ciekawostki, roczniki dla przeglądarki
  discover-bdl.mjs  pomocnik do wyszukiwania identyfikatorów zmiennych BDL
data/raw/           surowe pobrania (niewersjonowane)
public/dane/        dane serwowane stronie (nieweresjonowane, generowane)
src/pages/          Pulpit, Rejestr umów, Budżet, Statystyki, Porównaj, Źródła
...
- **Dwa źródła budżetu.** Szeregi GUS i tabele Urzędu Miasta są prezentowane obok siebie
